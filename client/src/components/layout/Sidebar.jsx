    ...
    <>
      ...
      <aside
        className={`
          ...
          bg-[var(--panel)] backdrop-blur-xl border-r border-[var(--border-subtle)]
      ...
      >
        <div className="flex h-16 items-center justify-between px-4 border-b border-[var(--border-subtle)]">
          {!collapsed && (
            <NavLink to="/" className="flex items-center gap-3 group" onClick={() => setMobileOpen(false)}>
              ...
              <span className="font-bold text-lg tracking-tight text-[var(--text-primary)] group-hover:text-[var(--accent-cyan)] transition-colors">FixMyCity</span>
            </NavLink>
          )}
          {collapsed && (
          ...
          <button
            className={`lg:hidden p-2 rounded-lg hover:bg-white/10 text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors ${collapsed ? 'hidden' : ''}`}
            onClick={() => setMobileOpen(false)}
          ...
          {navItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                onClick={() => setMobileOpen(false)}
                className={`
                  flex items-center gap-3 px-3 py-2.5 rounded-lg border transition-all duration-200 text-sm
                  ${active
                    ? 'bg-[var(--accent-cyan-dim)] text-[var(--accent-cyan)] font-semibold border-[var(--border-subtle)] shadow-[0_0_18px_rgba(34,211,238,0.12),inset_0_0_18px_rgba(34,211,238,0.05)]'
        ...
        {!collapsed && (
          <div className="p-3 border-t border-[var(--border-subtle)]">
            <div className="flex items-center gap-3 px-3 py-3 bg-white/5 border border-[var(--border-subtle)] rounded-xl">
              <Avatar
                name={user?.name}
                size="md"
                status="online"
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-[var(--text-muted)] capitalize">{roleLabels[user?.role] || user?.role}</p>
              </div>
            </div>
        ...
        {collapsed && (
          <div className="absolute bottom-4 left-0 right-0 px-3">
            <button
              onClick={handleLogout}
              className="w-full p-2 rounded-lg hover:bg-white/10 text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors"
              title="Logout"
            >
      ...
      {mobileOpen && (
        <button
          ...
          aria-label="Close sidebar"
        >
          <div className="w-12 h-12 rounded-full bg-[var(--bg-card-solid)] border border-[var(--border-subtle)] shadow-xl flex items-center justify-center">
